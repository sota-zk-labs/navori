module verifier_addr::fri_transform {
    ...
    fun transform_coset_of_size_4(
        ...
        let f0 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr);

        let f1 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 1);
        // f0 < 3P ( = 1 + 1 + 1).
        f0 = (f0 + f1 + fmul(
        ...
        let f2 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 2);
        let f3 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 3);

        f2 = (f2 + f3 + fmul(
            f2 + (K_MODULUS - f3),
            fmul(*vector::borrow(fri_ctx, fri_half_inv_group_ptr + 1), fri_eval_point_div_by_x)
        )) % K_MODULUS;
    ...
    fun transform_coset_of_size_8(
        ...
        let imaginary_unit = *vector::borrow(fri_ctx, fri_half_inv_group_prt + 1) ;
        ...
        let f4 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 4);
        let fri_eval_point_div_by_x2 = fmul(
            fri_eval_point_div_by_x,
            *vector::borrow(fri_ctx, fri_half_inv_group_prt + 2));

        let f5 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 5) ;

        f4 = (f4 + f5 + fmul(
            f4 + (K_MODULUS - f5),
            fri_eval_point_div_by_x2));

        let f6 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 6);
        let f7 = *vector::borrow(fri_ctx, evaluations_on_coset_ptr + 7);
